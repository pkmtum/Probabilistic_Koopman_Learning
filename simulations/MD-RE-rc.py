...
kmodel.save_weights("model/np_md_weights_rc_pre_res")
...
kmodel.load_weights("model/np_md_weights_rc_pre_res")
kmodel.train(epochs=600, batch_dim = ts_data.num_train_filtered)

kmodel.pred_data()
kmodel.plot_pred_train()

kmodel.save_weights("model/np_md_weights_rc_res")
...
pkmodel.init_with_mle_weights("model/np_md_weights_rc_res")
...
pkmodel.save_weights("model/p_md_weights_rc_res")
...
np.save("mlu/mlu-rc-res",np.array([m,l,u]))

# Save predictions, ELBO training curve and ELBO distribution
pkmodel.save_sample_preds(2000, "plot_data/MD_res-rc-")
np.save("plot_data/md_res-rc-hist", pkmodel.history.history['loss'])
elbos = [np.mean(pkmodel.train_model.loss(pkmodel.y, pkmodel.train_model(ts_data.x_inp))) for i in range(50)]
np.save("plot_data/md_res-rc-elbos", elbos)