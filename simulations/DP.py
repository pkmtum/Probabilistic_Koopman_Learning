...
kmodel.save_weights("model/pre-np-dp")
...
pkmodel.save_weights("model/p-dp")
...
pkmodel.save_sample_preds(2000, "plot_data/DP-")
np.save("plot_data/dp-hist", pkmodel.history.history['loss'])
elbos = [np.mean(pkmodel.train_model.loss(pkmodel.y, pkmodel.train_model(ts_data.x_inp))) for i in range(50)]
np.save("plot_data/dp-elbos", elbos)
