...
kmodel.save_weights("model/np_md_weights_ring_pre")
...
kmodel.load_weights("model/np_md_weights_ring_pre")
kmodel.train(epochs=30, batch_dim = ts_data.num_train_filtered)

kmodel.pred_data()
kmodel.plot_pred_train()

kmodel.save_weights("model/np_md_weights_ring")
...
pkmodel.init_with_mle_weights("model/np_md_weights_ring")
...
pkmodel.save_weights("model/p_md_weights_ring")
...
np.save("mlu/mlu-r",np.array([m,l,u]))

# Save predictions, ELBO training curve and ELBO distribution
pkmodel.save_sample_preds(2000, "plot_data/MD-ring-")
np.save("plot_data/md-ring-hist", pkmodel.history.history['loss'])
elbos = [np.mean(pkmodel.train_model.loss(pkmodel.y, pkmodel.train_model(ts_data.x_inp))) for i in range(50)]
np.save("plot_data/md-ring-elbos", elbos)